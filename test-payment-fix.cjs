  ...
  try {
    ...
    await client.query(`
      INSERT INTO payment_transactions (
        ...
        state, currency, effective_date
      ) VALUES (
        $1, $2, 'wire', 'TEST-WIRE', $3,
        'validated', 'USD', CURRENT_DATE
      )
    ...
    await client.query(`
      INSERT INTO outbox (
        ...
        'payment', $1, $2, 'payment.wire.validated', 
        $3, '{}', NOW()
      )
    `, [
      paymentId,
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (stateResult.rows[0]?.state === 'posted_pending_settlement') {
        success = true;
        break;
      }
    ...
    if (success) {
      ...
      console.log('  State: posted_pending_settlement');
      console.log('  Ledger entries created:', ledgerResult.rows[0].count);
      console.log('\n🎉 The fix is working! Manual payments will now create ledger entries.');
      console.log('=' .repeat(60));
    } else {
